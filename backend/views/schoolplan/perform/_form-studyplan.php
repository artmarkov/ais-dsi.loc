...
$readonly = (\artsoft\Art::isFrontend() && in_array($model->status_sign, [1, 2])) ? false : $readonly;
?>
...
//                        if ($model->isNewRecord) { // открыл для смены статусов, чтобы работал Yii::$app->request->post()
                            echo Html::activeHiddenInput($model, "studyplan_id");
//                        }
                        ?>
                        <?= $form->field($model, 'schoolplan_id')->widget(\kartik\select2\Select2::class, [
                            'data' => \common\models\schoolplan\Schoolplan::getSchoolplanListForStudyplan($model->studyplan_id),
                            'options' => [
                                'id' => 'schoolplan_id',
                        ...
                        <?= $form->field($model, 'teachers_id')->widget(\kartik\depdrop\DepDrop::class, [
                            'data' => \common\models\schoolplan\Schoolplan::getExecutorsListByName($model->studyplan_id, $model->schoolplan_id),
                            'options' => [
                                'id' => 'teachers_id',
                                'disabled' => /*$model->schoolplan->isExecutors() ? true :*/ $readonly,
                            ...
                            'pluginOptions' => [
                                ...
                                'placeholder' => Yii::t('art', 'Select...'),
                                'url' => Url::to(['/schoolplan/default/executors-perform', 'studyplan_id' => $model->studyplan_id])
                            ],
                        ])->label(Yii::t('art/teachers', 'Teacher'));
                        ?>
                        <?= $form->field($model, 'studyplan_subject_id')->widget(\kartik\depdrop\DepDrop::className(), [
                            'data' => \common\models\studyplan\Studyplan::getStudyplanSubjectListByStudyplanAndTeachers($model->studyplan_id, $model->teachers_id),
                            'options' => [
                            ...
                            'pluginOptions' => [
                                'depends' => ['teachers_id'],
                                'placeholder' => Yii::t('art', 'Select...'),
                                'url' => Url::to(['/schoolplan/default/studyplan-subject-teachers-perform', 'studyplan_id' => $model->studyplan_id])
                            ]
                        ]);
                        ?>
