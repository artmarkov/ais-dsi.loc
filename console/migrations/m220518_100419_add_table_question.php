...
class m220518_100419_add_table_question extends \artsoft\db\BaseMigration
{
    ...
    public function up()
    {
        ...
        $this->createTableWithHistory('question', [
            ...
            'category_id' => $this->integer()->notNull()->comment('Категория формы (Опрос, Заявка)'),
            'users_cat' => $this->integer()->notNull()->comment('Группа пользователей (Сотрудники, Преподаватели, Ученики, Родители, Гости)'),
            'vid_id' => $this->integer()->notNull()->comment('Вид формы (Открытая, Закрытая)'),
            'division_list' => $this->string(1024)->notNull()->comment('Список отделений'),
            'description' => $this->string(1024)->comment('Описание формы'),
