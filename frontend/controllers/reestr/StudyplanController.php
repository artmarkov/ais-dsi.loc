...
class StudyplanController extends MainController
{
    ...
    public function actionScheduleItems($id, $objectId = null, $mode = null)
    {
        ...
        $this->view->params['breadcrumbs'][] = ['label' => Yii::t('art/studyplan', 'Individual plans'), 'url' => ['reestr/studyplan/index']];
        $this->view->params['breadcrumbs'][] = ['label' => sprintf('#%06d', $id), 'url' => ['reestr/studyplan/view', 'id' => $id]];
        $this->view->params['tabMenu'] = $this->getMenu($id);
...
//                'studyplan_subject_id' => $studyplan_subject_id,
    ...
    public function actionStudyplanInvoices($id, $objectId = null, $mode = null)
    {
        ...
        if ('create' == $mode) {
            $this->view->params['breadcrumbs'][] = ['label' => Yii::t('art/studyplan', 'Studyplan Invoices'), 'url' => ['reestr/studyplan/default/studyplan-invoices', 'id' => $model->id]];
            $this->view->params['breadcrumbs'][] = 'Добавление карточки';
...
//                'model' => $model,
            ...
            $this->view->params['breadcrumbs'][] = ['label' => Yii::t('art/studyplan', 'Studyplan Invoices'), 'url' => ['reestr/studyplan/studyplan-invoices', 'id' => $id]];
            ...
            if ('view' == $mode) {
                $this->view->params['breadcrumbs'][] = ['label' => Yii::t('art/studyplan', 'Studyplan Invoices'), 'url' => ['reestr/studyplan/studyplan-invoices', 'id' => $model->id]];
                $this->view->params['breadcrumbs'][] = sprintf('#%06d', $objectId);
                $modelStudyplanInvoices = StudyplanInvoices::findOne($objectId);
            ...
            if ($model_date->studyplan_id != $id) {
                $this->redirect(['/reestr/studyplan/' . $model_date->studyplan_id . '/studyplan-invoices']);
            }
...
//            ['label' => 'Нагрузка', 'url' => ['/reestr/studyplan/load-items', 'id' => $id]],
            ...
            ['label' => 'Расписание консультаций', 'url' => ['/reestr/studyplan/consult-items', 'id' => $id]],
            ['label' => 'Характеристики по предметам', 'url' => ['/reestr/studyplan/characteristic-items', 'id' => $id]],
            ['label' => 'Тематические/репертуарные планы', 'url' => ['/reestr/studyplan/thematic-items', 'id' => $id]],
            ['label' => 'Дневник успеваемости', 'url' => ['/reestr/studyplan/studyplan-progress', 'id' => $id]],
            ['label' => 'Оплата за обучение', 'url' => ['/reestr/studyplan/studyplan-invoices', 'id' => $id]],
//            ['label' => 'Выполнение плана и участие в мероприятиях', 'url' => ['/reestr/studyplan/studyplan-perform', 'id' => $id]],
        ];
    }
}