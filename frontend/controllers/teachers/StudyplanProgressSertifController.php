...
/**
 * StudyplanProgressSertifController
 */
class StudyplanProgressSertifController extends MainController
{
    ...
    public function actionUpdate($objectId)
    {
        ...
        $this->view->params['breadcrumbs'][] = sprintf('#%06d', $this->teachers_id);

        $model = new AttestationItems();
        $modelsItems = AttestationItems::getAttestationsGroupForTeachers($this->teachers_id, $subject_key);
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($valid) {
                ...
                try {
                    ...
                    if ($flag) {
                        $transaction->commit();
                        $this->redirect(['/teachers/studyplan-progress', 'id' => $this->teachers_id]);
                    }
                } catch
                (Exception $e) {
