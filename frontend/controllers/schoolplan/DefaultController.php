...
//            print_r($model->errors);
    ...
    public function actionPerform($id, $objectId = null, $mode = null, $readonly = false)
    {
        ...
        if ('create' == $mode) {
            ...
            $modelPerform->teachers_id = $model->isExecutors() ? $this->teachers_id : null;
            ...
            $this->view->params['breadcrumbs'][] = ['label' => Yii::t('art/guide', 'Schoolplan Protocol'), 'url' => ['schoolplan/default/perform', 'id' => $id]];
            $this->view->params['breadcrumbs'][] = ['label' => sprintf('#%06d', $objectId), 'url' => ['schoolplan/default/perform', 'id' => $id, 'objectId' => $objectId, 'mode' => 'update']];
...
//            print_r($modelPerform->errors);
            if ($modelPerform->load(Yii::$app->request->post()) && $modelPerform->validate()) {
                if (Yii::$app->request->post('submitAction') == 'send_approve') {
                    ...
                    if ($modelPerform->sendApproveMessage()) {
                        Yii::$app->session->setFlash('info', Yii::t('art/mailbox', 'Your mail has been posted.'));
                    }
    ...
    public function actionStudyplanSubjectPerform()
    {
        $out = [];
        if (isset($_POST['depdrop_parents'])) {
            $parents = $_POST['depdrop_parents'];
    ...
    public function actionStudyplanThematicPerform()
    {
        $out = [];
        if (isset($_POST['depdrop_parents'])) {
            $parents = $_POST['depdrop_parents'];
    ...
    public function getMenu($id)
    {
        ...
        return [
            ...
            ['label' => 'Выполнение плана и участие в мероприятии', 'url' => ['/schoolplan/default/perform', 'id' => $id], 'visible' => $model->category->perform_flag && User::hasRole(['teacher','department'])],
            ['label' => 'Протокол аттестационной комиссии', 'url' => ['/schoolplan/default/protocol', 'id' => $id], 'visible' => $model->category->protocol_flag && User::hasRole(['teacher','department'])],
